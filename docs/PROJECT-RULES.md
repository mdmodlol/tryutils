...
- 如果是业务运营型任务，默认先做 weekly review，再进入实现
- 如果 GSC 数据不在标准目录，先复制到 `data/gsc-snapshots/YYYY-MM-DD/`
...
- 业务运营型任务同样要先写计划
- 业务运营型任务默认不直接改首页结构或扩品类
- 业务运营型任务的标准输入是：
  - `data/gsc-snapshots/YYYY-MM-DD/`
  - `docs/analytics/latest-gsc-summary.md`
- 业务运营型任务的标准输出是：
  - `docs/analytics/weekly/YYYY-MM-DD.md`
  - 更新后的 `docs/session-state.md`
- 如果只是做 weekly review，可以不跑 `pnpm build`
- 如果 weekly review 之后进入代码或内容实现，仍然按固定验收链执行
...
- 如果只跑了部分检查，不能当作任务完成
- `pnpm gsc:summary` 用于把标准 GSC 快照汇总成仓库内可复用的 summary
